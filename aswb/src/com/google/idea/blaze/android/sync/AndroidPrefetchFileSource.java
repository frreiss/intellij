...
public class AndroidPrefetchFileSource
  ...
  private static Collection<ArtifactLocation> getAndroidSources(TargetIdeInfo target) {
    ...
    if (target.getAndroidAarIdeInfo() != null) {
      fileSet.add(target.getAndroidAarIdeInfo().getAar());
    }
    ...
    if (androidIdeInfo != null) {
      ...
      collectJarsFromLibrary(androidIdeInfo.getIdlJar(), fileSet);
    }

    JavaIdeInfo javaIdeInfo = target.getJavaIdeInfo();
    if (javaIdeInfo != null) {
      LibraryArtifact filteredGenJar = javaIdeInfo.getFilteredGenJar();
      collectJarsFromLibrary(filteredGenJar, fileSet);
      for (LibraryArtifact generatedJar : javaIdeInfo.getGeneratedJars()) {
        collectJarsFromLibrary(generatedJar, fileSet);
      }
      for (LibraryArtifact jar : javaIdeInfo.getJars()) {
        collectJarsFromLibrary(jar, fileSet);
      }
    }
  ...
  /** Collect all jar files of a LibraryArtifact to be cached. */
  private static void collectJarsFromLibrary(
      LibraryArtifact libraryArtifact, Set<ArtifactLocation> toCache) {
    if (libraryArtifact != null) {
      if (libraryArtifact.getClassJar() != null) {
        toCache.add(libraryArtifact.getClassJar());
      }
      if (libraryArtifact.getInterfaceJar() != null) {
        toCache.add(libraryArtifact.getInterfaceJar());
      }
      toCache.addAll(libraryArtifact.getSourceJars());
    }
  }
}
